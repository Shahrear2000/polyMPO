...
void MPMesh::reconstruct_coeff_full(){
  ...
  this->vtxMatrixMass = vtxMatrixMass_l;
    ...
    pumipic::RecordTime(
        "VR Invert Matrix " + std::to_string(self),
        timer.seconds());

    Kokkos::fence();

    postCommunicationComputeTime = phaseTimer.seconds();
    MPI_Barrier(comm);
    postCommunicationComputeImbalance =
        phaseTimer.seconds() - postCommunicationComputeTime;

    const double computeTime =
        preCommunicationComputeTime + postCommunicationComputeTime;

    const double communicationTime =
        gatherCommunicationTime + scatterCommunicationTime;

    pumipic::RecordTime(
        "Reconstruction_Scatter_Communication_" + std::to_string(self),
        scatterCommunicationTime);

    pumipic::RecordTime(
        "Reconstruction_Scatter_Communication_Imbalance_" + std::to_string(self),
        scatterCommunicationImbalance);

    pumipic::RecordTime(
        "Reconstruction_PostComm_Compute_" + std::to_string(self),
        postCommunicationComputeTime);

    pumipic::RecordTime(
        "Reconstruction_PostComm_Compute_Imbalance_" + std::to_string(self),
        postCommunicationComputeImbalance);

    pumipic::RecordTime(
        "Reconstruction_Compute_Total_" + std::to_string(self),
        computeTime);

    pumipic::RecordTime(
        "Reconstruction_Communication_Total_" + std::to_string(self),
        communicationTime);

    pumipic::RecordTime(
        "Reconstruction_Total_" + std::to_string(self),
        totalTime);
